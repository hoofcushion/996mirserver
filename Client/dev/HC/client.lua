...
function Callback.register(uid, callback)
	Callback.handlers[uid] = { os.time(), callback }
end

function Callback.trigger(uid)
	local handlers = Callback.handlers
	...
	return setmetatable({}, {
		...
		__call = function(_, ...)
			Common.log(
				...
				actor)
			Common.sendmsg(Msg.call, Msg.call_call, 0, 0, Json.encode({ keys, { ... } }), actor)
		end,
	})
end
local function anode(keys, actor)
	return setmetatable({}, {
		...
		__call = function(_, ...)
			...
			Common.sendmsg(Msg.call, Msg.call_call, 0, uid, Json.encode({ keys, { ... } }), actor)
			Await(function(callback)
				Callback.register(uid, callback)
			end)
		end,
	})
end
...
Server = HC.Cache.table(function(k)
	return node({ k }, Meta.MAIN_ACTOR_ID)
end)
---@class Server
AServer = HC.Cache.table(function(k)
	if not is_async() then return end
	return anode({ k }, Meta.MAIN_ACTOR_ID)
end)
-- client api register
---@class Client
Client = {}
local hooks
SL.NetworkUtil.OnLuaMessage, hooks = HC.hooks(SL.NetworkUtil.OnLuaMessage)
hooks.handler = function(_, ...)
	Event.push(Reg.handlerequest, ...)
end
Event.register(Reg.handlerequest, {
	fn = function(msgid, action, _, uid, data)
		if msgid == Msg.call then
			...
			if uid ~= 0 then
				Common.sendmsg(Msg.call_callback, 0, 0, uid, "", actor)
			end
		elseif msgid == Msg.call_callback then
			Common.log(("Client callback: %s"):format(uid), actor)
			Callback.trigger(uid)
		end
	end,
})
local _ = SL:Schedule(function()
	local handlers = Callback.handlers
	for uid, v in pairs(handlers) do
		if os.time() - v[1] > 10 then
			v[2](false)
			handlers[uid] = nil
		end
	end
...
Event.register(Reg.handlerequest, {
	fn = function(msgid, _, _, _, code)
		if Msg.runcode == msgid then
			HC.runcode(code)
		end
	end,
})
...
function WindowAnimate(window)
	GUI:setScale(window, 0.9)
	window:runAction(GUI:ActionEaseBackOut(GUI:ActionScaleTo(0.2, 1)))
	for _, v in WindowWalk(window) do
		local orig = GUI:getOpacity(v)
		GUI:setOpacity(v, 0)
		GUI:Timeline_FadeTo(v, orig, 0.2)
	end
end

--- 遍历包括节点自身在内的所有子节点
function WindowWalk(window)
	local function yield(v, p)
		if GUI:Win_IsNotNull(v) then
			coroutine.yield(v, p)
		end
	end
	local function walk(file, path)
		for _, v in ipairs(GUI:getChildren(file)) do
			local filepath = path .. "/" .. GUI:getName(v)
			yield(filepath, v)
			walk(v, filepath)
		end
	end
	return coroutine.wrap(function()
		local name = GUI:getName(window)
		yield(name, window)
		walk(window, name)
	end)
end
...
local ProxyNode = {}
---@return ProxyNode
local function WindowProxyNode(keys, window)
	return setmetatable({}, {
		__index = function(_, k)
			assert_type("key", k, "string")
			local new_keys = HC.deepcopy(keys)
			table.insert(new_keys, k)
			return WindowProxyNode(new_keys, window)
		end,
		__call = function()
			local r = window
			for _, k in ipairs(keys) do
				r = GUI:getChildByName(r, k)
				if r == nil then
					return
				end
			end
