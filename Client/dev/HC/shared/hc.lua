...
---@param num integer
---@return fun(n:integer,p:integer):(0|1), integer, integer
function HC.forbit(num)
	return _forbit,num,0
end
local function _sanepairs(e)
	if e.i then
		e.m=e.i
		e.i=e.i+1
		if e.t[e.i] then
...
--- sane pairs iterator
--- always iterate the array keys first
...
local function _flatten(tbl,ret)
...
---@param tbl table
---@return table
function HC.flatten(tbl)
	return _flatten(tbl,{})
end
---@generic T
---@param tbl T
---@return T
function HC.unique(tbl)
	local exist={}
	local ret={}
	for _,v in pairs(tbl) do
		if not exist[v] then
...
---@generic T
---@param tbl1 table<any,T>
---@param tbl2 table<any,T>
---@return T[]
function HC.difference(tbl1,tbl2)
	local ret={}
	local seen={}
...
---@param tbl2 table<any,T>
---@return T[]
function HC.intersection(tbl1,tbl2)
	local ret={}
	local seen={}
	for _,v in pairs(tbl2) do
		seen[v]=true
...
---@param fini integer
---@return T[]
function HC.slice(tbl,start,fini)
	local ret={}
	table.move(tbl,start,fini,1,ret)
	return ret
end
--- ---
--- General functions
