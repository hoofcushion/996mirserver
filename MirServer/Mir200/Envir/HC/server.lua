	...
	fn = function(actor, msgid, action, _, uid, data)
		if msgid == Msg.call then
			...
			if action == Msg.call_newindex then
				Common.log(("Suspicious newindex call: %s=%s"), actor)
		...
		elseif msgid == Msg.call_callback then
			Common.log(("Server callback: %s"):format(uid), actor)
			Callback.trigger(uid)
		end
	end,
})
Export.callbackgc = unknown(function()
	local handlers = Callback.handlers
...
addscheduled("callbackgc", "SEC", "1", Export.callbackgc)
